...
import { imageUploadHandler, ImageUploadPayload } from './handlers/imageUpload';
import { indexWardrobeHandler, IndexWardrobePayload } from './handlers/indexWardrobe';
import {
  sendFeedbackRequestHandler,
  SendFeedbackRequestPayload,
} from './handlers/sendFeedbackRequest';
import { storeMemoriesHandler, StoreMemoriesPayload } from './handlers/storeMemories';
...
const withTaskLifecycle =
  <T>(handler: (prisma: PrismaClient, payload: T) => Promise<unknown>): HttpFunction =>
  async (req, res) => {
    ...
    try {
      ...
      if (task.status !== 'QUEUED') {
        ...
        res.status(400).send({ message: `Task in invalid state: ${task.status}` });
        return;
      }
...
const imageUploadFunction = withTaskLifecycle<ImageUploadPayload>(async (prisma, payload) => {
  const validated = validatePayload<ImageUploadPayload>(payload, ['userId', 'messageId']);
  return imageUploadHandler(prisma, validated);
});

const storeMemoriesFunction = withTaskLifecycle<StoreMemoriesPayload>(async (prisma, payload) => {
  const validated = validatePayload<StoreMemoriesPayload>(payload, ['userId', 'conversationId']);
  return storeMemoriesHandler(prisma, validated);
});

const indexWardrobeFunction = withTaskLifecycle<IndexWardrobePayload>(async (prisma, payload) => {
  const validated = validatePayload<IndexWardrobePayload>(payload, ['userId', 'messageId']);
