...
const logger = getLogger('service:wardrobe');
...
export async function processDueWardrobeJobs(maxBatch: number = 50): Promise<void> {
  ...
  for (const messageId of dueMessageIds) {
    ...
    try {
      ...
      logger.error({ messageId, err: err?.message }, 'Failed processing wardrobe job');
    } finally {
...
async function indexWardrobeFromMessage(messageId: string): Promise<void> {
  ...
  try {
    ...
    logger.debug({ messageId, status: output.status }, 'Wardrobe indexing completed');
    ...
    logger.error({ messageId, err: err?.message }, 'Wardrobe indexing failed');
