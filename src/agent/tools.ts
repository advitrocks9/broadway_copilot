...
export function searchWardrobe(userId: string): Tool {
  const searchWardrobeSchema = z.object({
    query: z
      .array(z.string())
      .describe(
        "A list of natural language queries to search for clothing items. Each query should describe an item of clothing.",
      ),
  });
  ...
  return new Tool({
    name: "searchWardrobe",
    description:
      "Searches the user's digital wardrobe for clothing items based on a list of descriptive queries. Useful for finding specific items (e.g., 'red summer dress') or items for an outfit (e.g., ['white t-shirt', 'blue jeans']). Returns a list of matching items with their details.",
    ...
    func: async ({ query }: z.infer<typeof searchWardrobeSchema>) => {
      ...
      try {
        ...
        logger.error(
          { userId, query, err: (err as Error)?.message },
...
export function fetchRelevantMemories(userId: string): Tool {
  const fetchRelevantMemoriesSchema = z.object({
    query: z
      .string()
      .describe(
        'A natural language query describing the information you want to find about the user. For example: "user\'s favorite color" or "upcoming vacation destination".',
      ),
  });
  ...
  return new Tool({
    ...
    description:
      "Searches the user's personal profile and past conversations for relevant information. Use this to recall user preferences, style, or any other personal details they have shared.",
    ...
    func: async ({ query }: z.infer<typeof fetchRelevantMemoriesSchema>) => {
      ...
      try {
        ...
        const memories: { id: string; memory: string }[] =
