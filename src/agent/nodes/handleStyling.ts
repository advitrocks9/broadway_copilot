...
export async function handleStyling(state: GraphState): Promise<GraphState> {
  ...
  if (!stylingIntent) {
    throw new InternalServerError(
      "handleStylingNode called without a styling intent.",
    );
  }

  try {
    if (lastMessage?.meta?.buttonPayload) {
      const defaultPromptText = await loadPrompt(
        "handlers/styling/handle_styling_no_input.txt",
      );
      const systemPromptText = defaultPromptText.replace(
        "{INTENT}",
        stylingIntent,
      );
    ...
    const systemPromptText = await loadPrompt(
      `handlers/styling/handle_${stylingIntent}.txt`,
    );
    const systemPrompt = new SystemMessage(systemPromptText);
