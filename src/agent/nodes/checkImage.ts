 ...
 */
export async function checkImageNode(state: { input: RunInput; intent?: string }): Promise<{ next?: string; reply?: { reply_type: 'text'; reply_text: string } }> {
  ...
  if (!target) {
    ...
    return { next: 'send_reply', reply: { reply_type: 'text', reply_text: 'Tell me what you need help with.' } };
  }
  ...
  return { next: 'send_reply', reply: { reply_type: 'text', reply_text: askText } };
}
